function [integration_error, Nt, ft_5, integral_1000] = zadanie3()
    ...
    integration_result = [];
	for i = 1:length(Nt)
		integration_result(i) = integral(@f, Nt(i));  % ałka dla bieżącej liczby podprzedziałów
	...
	title('Error of Composite Simpsons rule integration method depending on the number of subintervals');
	grid on;
...
function res = integral (f, N)  % Całkowanie metodą Simpsonsona
	a = 0;
	b = 5;
	h = (b-a)/N;
	res = 0;
	...
	res = res*h/6;  % Suma * szerokość podprzedziałów / 6
    %%%%%
end
%%%%%