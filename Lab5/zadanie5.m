function [M,N,P,R,x_coarse,y_coarse,F_coarse,x_fine,y_fine,F_fine] = zadanie5()
    close all;
    P = 199; % liczba unikalnych współrzędnych x punktów dla których będzie obliczana interpolacja
    R = 199; % liczba unikalnych współrzędnych y punktów dla których będzie obliczana interpolacja
    M = 19; % liczba węzłów interpolacji wzdłuż osi X (stopień wielomianu zmiennej x: M-1)
    N = 19; % liczba węzłów interpolacji wzdłuż osi Y (stopień wielomianu zmiennej y: N-1)
    ...
    F_coarse = sin(X_coarse*2*pi) .* abs(Y_coarse-0.5); % zadanie A: zmień definicję funkcji interpolowanej
